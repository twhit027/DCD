...
class Admin extends Database
{
	...
	function insertListings()
	{
		...
		for($i=0;$i<$usercount; $i++) 
		{
			...
			$stmt = $this->db->prepare("INSERT INTO `listing` (`ID`, `StartDate`, `EndDate`, `Placement`,`Position`, `AdText`, `SiteCode`, `Street`, `City`, `State`, `Zip`) VALUES(:ID, :StartDate, :EndDate, :Placement, :Position, :AdText, :Site, :Street, :City, :State, :Zip)");						
	...
	{
		$date = date("Y-m-d");	
		$stmt = $this->db->prepare("DELETE FROM `listing` WHERE `EndDate` < :date ");	
	...
	function buildNav()
	{		
		$stmt = $this->db->prepare("TRUNCATE TABLE `position`");
		$del = $stmt->execute();
