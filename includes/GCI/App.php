...
class App
{
    ...
    {
        $sql = "SELECT ID, AdText, SiteCode, Placement, Position, Images, Street, City, State, Zip, Rent, Amenities, BathRooms, BedRooms, Deposit, Email, Pets, Phone, ExerciseRec, CommFeat, Neighborhood, Parking, PropType, SquareFeet FROM `listing` where ID = :id";
        $params = array(':id' => $id);
        $results = $this->database->getAssoc($sql, $params);
